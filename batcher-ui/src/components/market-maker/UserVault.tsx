...
import { getUserVault } from '@/actions';
import { selectUserVault, userAddressSelector } from '@/reducers';
...
const UserVault = () => {
  ...
  const userVault = useSelector(selectUserVault);

  useEffect(() => {
    if (userAddress) dispatch(getUserVault(userAddress));
        ...
        {userVault && (
          <div className="flex grow flex-col justify-center md:flex-row p-3 border-solid border-2 border-lightgray my-2">
            <div className="p-3">
              ...
              <p>{`Shares: ${userVault.shares}`}</p>
              {`Unclaimed Rewards: ${userVault.unclaimed} TEZ`}
              {/* {showAddLiquidity({
