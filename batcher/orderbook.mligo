...
let right_partial_match
  ...
  (treasury : CommonTypes.Types.treasury) : treasury * matching * matching =
    ...
    let token_holding_2 = CommonTypes.Utils.token_amount_to_token_holding ord_2.trader new_token_amount_2 in
...
let left_partial_match
  ...
  (treasury : CommonTypes.Types.treasury) : treasury * matching * matching =
      ...
      let remaining_1_amount = Math.get_rounded_number (Float.div (Float.sub (float_of_equivalent_ord_1_amount) (float_of_ord_2_amount)) rate) in
      // let amount_of_1_to_be_swapped = abs (ord_1.swap.from.amount - remaining_1_amount) in
      let amount_of_1_to_be_swapped = Math.get_rounded_number (Float.div float_of_ord_2_amount rate) in  
...
let match_orders
  ...
  match_calculation equivalent_ord_1_amount ord_1 ord_2 exchange_rate.rate treasury
