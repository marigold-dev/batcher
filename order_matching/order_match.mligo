    ...
    let f =
      fun (order, (storage, l) : order * (storage * orderlist)) ->
        if is_expired order then
          ...
          (storage, order :: l) in
    let (new_storage, new_bids) = List.fold_right f bids (storage,([] : orderlist)) in
    let (new_storage, new_asks) = List.fold_right f asks (new_storage,([] : orderlist)) in
      ...
      | bid :: bids, ask :: asks ->
        if bid.to_price < ask.to_price then
          ...
          if bid.to_price > ask.to_price then
            ...
            (match (compute_match_type bid ask) with
